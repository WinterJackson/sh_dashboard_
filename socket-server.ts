...
io.on("connection", (socket: AuthenticatedSocket) => {
    ...
    const resetActivityTimer = () => {
        ...
        socket.activityTimer = setTimeout(() => {
            if (socket.user?.id) {
                socket.broadcast.emit('user-status-changed', { userId: socket.user.id, status: 'away' });
            }
        }, 5 * 60 * 1000); // 5 minutes
    };
    ...
    // Message Status
    socket.on('message-delivered', async ({ conversationId, messageId, userId }) => {
        try {
            await prisma.message.update({
                where: { messageId },
                data: { deliveredAt: new Date() },
            });
            io.to(conversationId).emit('message-was-delivered', { messageId, userId });
        } catch (error) {
    ...
    socket.on('react-to-message', async ({ conversationId, messageId, userId, reaction }) => {
        resetActivityTimer();
        try {
            ...
            if (message) {
                ...
                io.to(conversationId).emit('message-reacted', updatedMessage);
            }
        } catch (error) {
    ...
    socket.on('edit-message', async ({ messageId, newContent, conversationId }) => {
        ...
        try {
            ...
            if (message && message.senderId === socket.user?.id) {
                ...
                io.to(conversationId).emit('message-edited', updatedMessage);
            }
        } catch (error) {
    ...
    socket.on('delete-message', async ({ messageId, conversationId }) => {
        ...
        try {
            ...
            if (message && (message.senderId === socket.user?.id || ['ADMIN', 'SUPER_ADMIN'].includes(socket.user?.role))) {
    ...
    socket.on('call-user', async (data) => {
        ...
        const callerName = fromUser?.profile?.firstName ? `${fromUser.profile.firstName} ${fromUser.profile.lastName}` : fromUser?.username || 'Unknown User';
    ...
    socket.on("disconnect", () => {
        if (socket.activityTimer) clearTimeout(socket.activityTimer);
        if (socket.user?.id) {
            socket.broadcast.emit('user-status-changed', { userId: socket.user.id, status: 'offline' });
        }
