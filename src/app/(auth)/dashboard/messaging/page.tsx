...
import ChatWindowSkeleton from "@/components/messaging/ChatWindowSkeleton";
...
const MessagingPage = () => {
    ...
    useEffect(() => {
        ...
        socket.on('message-seen', (newReceipt: any) => {
            queryClient.setQueryData(['messages', newReceipt.conversationId], (oldData: any) => {
                if (!oldData) return oldData;
                const newPages = oldData.pages.map((page: Message[]) =>
                    page.map((m: Message) =>
                        m.messageId === newReceipt.messageId
                            ? { ...m, readReceipts: [...(m.readReceipts || []), newReceipt] }
                            : m
                    )
                );
                return { ...oldData, pages: newPages };
        ...
        socket.on("message-edited", (updatedMessage: Message) => {
            queryClient.setQueryData(['messages', updatedMessage.conversationId], (oldData: any) => {
                if (!oldData) return oldData;
                const newPages = oldData.pages.map((page: Message[]) =>
                    page.map((m: Message) =>
