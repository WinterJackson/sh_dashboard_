// src/components/appointments/ui/appointments-table/AppointmentsFilters.tsx

"use client";

import { useSearch } from "@/app/context/SearchContext";
import { DatePicker } from "@/components/appointments/ui/appointments-table/DatePicker";
import { DateRangePicker } from "@/components/appointments/ui/appointments-table/DateRangePicker";
import {
    DropdownMenu,
    DropdownMenuContent,
    DropdownMenuItem,
...
import { CircleX as CancelledIcon, ChevronRight } from "lucide-react";
...
const AppointmentsFilters: React.FC<AppointmentsFiltersProps> = ({
    ...
    const activeFilters = useMemo(
        () => [
            ...
            ...(appointmentTypeFilter
                ? [
                      {
                          label: appointmentTypeFilter,
                          onClear: () => setAppointmentTypeFilter(null),
                      },
                  ]
                : []),
            ...
            ...(dateFilter
                ? [
                      {
                          label: dateFilter.toLocaleDateString(),
            ...
            ...(dateRangeFilter
                ? [
                      {
                          label: `${dateRangeFilter.from?.toLocaleDateString()} - ${dateRangeFilter.to?.toLocaleDateString()}`,
    ...
    const filteredAppointments = useMemo(() => {
        return appointments.filter((appointment) => {
                ...
                switch (filterType) {
                    ...
                    case "Hospital":
                        if (
                            ...
                            filterMatch = appointment.hospital.hospitalName
                                .toLowerCase()
                                .includes(searchTextLower);
                        }
                        break;
                    ...
                    default:
                        ...
                        const isAgeMatch = appointment.patient?.user?.profile
                            ?.dateOfBirth
                            ? differenceInYears(
                                  new Date(),
                                  new Date(
                        ...
                        const isIdMatch = appointment.patient?.patientId
                            .toString()
                            .includes(searchTextLower);
                        const isDateMatch = new Date(
                        ...
                        const isTypeMatch = appointment.type
                            ?.toLowerCase()
                            .includes(searchTextLower);
                        const isHospitalMatch = appointment.hospital
                            ?.hospitalName
                            ? appointment.hospital.hospitalName
                                  .toLowerCase()
                                  .includes(searchTextLower)
            ...
            if (selectedHospital) {
                filterMatch =
                    filterMatch && appointment.hospitalId === selectedHospital;
            }
