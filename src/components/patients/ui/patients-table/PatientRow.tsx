// src/components/patients/ui/patients-table/PatientRow.tsx
...
import { FetchedPatient } from "@/lib/definitions";
...
const PatientRow: React.FC<PatientRowProps> = React.memo(
    ({ patient, onDelete, onSelect, isSelected }) => {
        const profile = patient.user?.profile;
        const user = patient.user;
        const router = useRouter();

        const getLastAppointment = (
            appointments: FetchedPatient["appointments"] | undefined
        ) => {
        ...
        return (
            <tr className="group bg-background shadow-md py-2 hover:bg-primary hover:text-primary-foreground">
                {/* Checkbox Column */}
                <td className="text-center w-[8%] p-2 rounded-l-xl">
                    <input
                        type="checkbox"
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div className="flex flex-col">
                                ...
                                <TooltipProvider>
                                    <Tooltip>
                                        <TooltipTrigger>
                                            <p className="text-sm text-primary group-hover:text-primary-foreground truncate max-w-52 sm:max-w-[250px]">
