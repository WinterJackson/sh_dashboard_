...
const OutwardReferralsCard = () => {
    ...
    useEffect(() => {
        ...
        const fetchReferrals = async () => {
            ...
            // console.log(filteredReferrals.length)

            const outwardReferrals = filteredReferrals.filter(
                (referral: { type: string }) => referral.type === "External"
            );

            // console.log(outwardReferrals)

            setOutwardReferrals(outwardReferrals.length);

            const today = new Date();
            const currentWeekReferrals: any[] = [];
            const previousWeekReferrals: any[] = [];

            outwardReferrals.forEach(
