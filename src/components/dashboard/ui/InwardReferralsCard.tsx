// File: src/components/dashboard/ui/InwardReferralsCard.tsx
...
const InwardReferralsCard: React.FC<InwardReferralsCardProps> = ({
    ...
    const {
        ...
        const currentWeekReferrals = inwardReferrals.filter((referral) => {
            const diff =
        ...
        const previousWeekReferrals = inwardReferrals.filter((referral) => {
            const diff =
                (today.getTime() - new Date(referral.effectiveDate).getTime()) /
                (1000 * 3600 * 24);
